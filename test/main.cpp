...
#pragma comment( lib , "../3rdparty/debug/lib/glfw3")
...
#pragma comment( lib , "opengl32")
#pragma comment( lib , "../3rdparty/lib/glfw3")
...
glfw_render(GLFWwindow * wnd)
{
	...
	for (c = 0; c < BATCH_COUNT; c++)
	{
		...
		int sample_coubnt = r / 4;
		for (int iii = 0; iii < sample_coubnt; ++iii)
		{
			((float*)g_as->src_buf)[iii] = (rand() % 10000) / 10000.0;
		}
