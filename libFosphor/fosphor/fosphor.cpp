	...
	fosphor_init(void)
{
	...
	/* Allocate structure */
	self = (fosphor*)malloc(sizeof(struct fosphor));
	if (!self)
		return NULL;
	...
	{
		self->img_waterfall = (float *)malloc(FOSPHOR_FFT_LEN * 1024 * sizeof(float));
		self->img_histogram = (float *)malloc(FOSPHOR_FFT_LEN * 128 * sizeof(float));
		self->buf_spectrum = (float *)malloc(2 * 2 * FOSPHOR_FFT_LEN * sizeof(float));
