...
cl_device_query(cl_device_id dev_id, struct fosphor_cl_features *feat)
{
	...
	/* Device type */
	err = clGetDeviceInfo(dev_id, CL_DEVICE_TYPE, sizeof(cl_device_type), &feat->type, nullptr);
	if (err != CL_SUCCESS)
		return -1;

	/* Vendor */
	err = clGetDeviceInfo(dev_id, CL_DEVICE_VENDOR, sizeof(feat->vendor)-1, &feat->vendor, nullptr);
	if (err != CL_SUCCESS)
		return -1;

	/* Local memory size */
	err = clGetDeviceInfo(dev_id, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong), &feat->local_mem, nullptr);
	if (err != CL_SUCCESS)
		return -1;

	/* CL/GL extension */
	err = clGetDeviceInfo(dev_id, CL_DEVICE_EXTENSIONS, sizeof(txt)-1, txt, nullptr);
	if (err != CL_SUCCESS)
		return -1;
	...
	err = clGetDeviceInfo(dev_id, CL_DEVICE_VERSION, sizeof(txt)-1, txt, nullptr);
	if (err != CL_SUCCESS)
		return -1;
	...
	else if (!(feat->flags & (FLG_CL_OPENCL_11 | FLG_CL_LOCAL_ATOMIC_EXT)))
	{
		 ...
		 */
		err = clGetDeviceInfo(dev_id, CL_DEVICE_VENDOR, sizeof(txt)-1, txt, nullptr);
		if (err != CL_SUCCESS)
			return -1;
...
cl_find_device(cl_platform_id *pl_id_p, cl_device_id *dev_id_p,
               struct fosphor_cl_features *feat)
{
	cl_platform_id pl_list[MAX_PLATFORMS] = { 0 };
	cl_platform_id pl_id=0;
		...
		cl_int *err_ptr)
{
	...
	/* Grab resource */
	src = (const char *)resource_get(resource_name, nullptr);
	if (!src) {
	...
	prog = clCreateProgramWithSource(ctx, 1, (const char **)&src, nullptr, &err);
	CL_ERR_CHECK(err, "Failed to create program");

        /* Build it */
        err = clBuildProgram(prog, 0, nullptr, opts, nullptr, nullptr);
		...
		txt_buf = (char *)malloc(txt_buf_len);
...
#ifdef DEBUG_CL
	{
		...
		clGetProgramInfo(prog, CL_PROGRAM_BINARY_SIZES, sizeof(size_t), &bin_len, nullptr);
		fprintf(stderr, "Binary length for '%s': %d\n\n", resource_name, (int)bin_len);

		bin_buf = malloc(bin_len);

		clGetProgramInfo(prog, CL_PROGRAM_BINARIES, sizeof(char *), &bin_buf, nullptr );
...
cl_do_init(struct fosphor *self)
{
	...
	if (self->flags & FLG_FOSPHOR_USE_CLGL_SHARING)
	{
		...
		cl->ctx = clCreateContext(ctx_props, 1, &cl->dev_id, nullptr, nullptr, &err);
		if (err != CL_SUCCESS) {
	...
	/* FFT program/kernels */
	cl->prog_fft = cl_load_program(cl->dev_id, cl->ctx, "fft.cl", nullptr, &err);
	if (!cl->prog_fft)
		goto error;
...
fosphor_cl_init(struct fosphor *self)
{
	...
	/* Allocate structure */
	cl =(fosphor_cl_state *) malloc(sizeof(struct fosphor_cl_state));
	if (!cl)
		return -ENOMEM;
	...
	cl->state = fosphor_cl_state::CL_BOOTING;
	...
	err = clGetDeviceInfo(cl->dev_id, CL_DEVICE_NAME, sizeof(dev_name)-1, dev_name, nullptr);
                   ...
                   void *samples, int len)
{
	...
	err = clEnqueueNDRangeKernel(cl->cq, cl->kern_fft, 2, nullptr, global, local, 0, nullptr, nullptr);
	CL_ERR_CHECK(err, "Unable to queue FFT kernel execution");

	/* Capture all GL objects */
	if ((cl->state != fosphor_cl_state::CL_PENDING) && (self->flags & FLG_FOSPHOR_USE_CLGL_SHARING)) {
		cl_mem objs[3];
	...
	err = clEnqueueNDRangeKernel(cl->cq, cl->kern_display, 2, nullptr, global, local, 0, nullptr, nullptr);
	...
	/* New state */
	cl->state = fosphor_cl_state::CL_PENDING;
...
fosphor_cl_finish(struct fosphor *self)
{
	...
	if (cl->state == fosphor_cl_state::CL_READY)
		return 0;
	...
	if (self->flags & FLG_FOSPHOR_USE_CLGL_SHARING)
	{
		...
		err = clEnqueueReleaseGLObjects(cl->cq, 3, objs, 0, nullptr, nullptr);
	...
	/* New state */
	cl->state = fosphor_cl_state::CL_READY;
