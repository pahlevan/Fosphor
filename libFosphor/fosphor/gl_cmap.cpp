...
gl_cmap_init_shader(struct gl_cmap_shader *shader, const char *name)
{
	...
	/* Load shader sources */
	shader_src = (const char *)resource_get(name, NULL);
	if (!shader_src)
		return -ENOENT;
...
/* -------------------------------------------------------------------------- */
 ...
 fosphor_gl_cmap_ctx * fosphor_gl_cmap_init(void)
{
	...
	/* Allocate structure */
	cmap_ctx = (fosphor_gl_cmap_ctx *)malloc(sizeof(struct fosphor_gl_cmap_ctx));
	if (!cmap_ctx)
		return NULL;
...
fosphor_gl_cmap_generate(GLuint *cmap_id, gl_cmap_gen_func_t gfn, void *gfn_arg, int N)
{
	...
	/* Temp array for data */
	rgba = (uint32_t *)malloc(sizeof(uint32_t) * N);
	if (!rgba)
		return -1;
