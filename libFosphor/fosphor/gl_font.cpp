...
gl_font * glf_alloc(int height, int flags)
{
	struct gl_font *glf = nullptr;
	FT_Error ftr;

	glf = (gl_font *)calloc(1, sizeof(struct gl_font));
	if (!glf)
		return nullptr;
...
_glf_init_face(struct gl_font *glf)
{
	...
	/* Raw data array */
	data = (uint8_t *)calloc(glf->tex.width * glf->tex.height, glf->flags & GLF_FLG_LCD ? 3 : 1);
	if (!data)
		return -ENOMEM;
...
glf_load_face_mem(struct gl_font *glf, const void *data, size_t len)
{
	...
	ftr = (FT_Error)FT_New_Memory_Face(glf->library,(const FT_Byte*) data, len, 0, &glf->face);
	if (ftr)
		return -EINVAL;
	     ...
	     const char *str)
{
	...
	data = (float *)malloc(8 * sizeof(float) * 4 * strlen(str));
